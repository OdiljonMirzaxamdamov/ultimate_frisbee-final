...
export const loadState = () => {
  try {
    ...
    let {
      tournamentsList, user, games, teams, rosters, players, logs,
    } = loadedState;
    ...
    if (tournamentsList && tournamentsList.list && !Record.isRecord(tournamentsList)) {
      tournamentsList = convertPlainObjectToState(
        tournamentsList,
        TournamentData,
    ...
    if (user && user.userData && !Record.isRecord(user)) {
      user = convertPlainObjectToState(
        user,
        UserData,
        defaultUser,
        UserState,
    ...
    if (games && games.list && !Record.isRecord(games)) {
      games = convertPlainObjectToState(
        games,
        GameData,
        defaultGame,
        GamesState,
    ...
    if (teams && teams.list && !Record.isRecord(teams)) {
      teams = convertPlainObjectToState(
        teams,
        TeamData,
        defaultTeams,
        TeamsState,
    ...
    if (rosters && rosters.list && !Record.isRecord(rosters)) {
      rosters = convertPlainObjectToState(
        rosters,
        RosterData,
        defaultRosters,
        RostersState,
    ...
    if (players && players.list && !Record.isRecord(players)) {
      players = convertPlainObjectToState(
        players,
        PlayerData,
        defaultPlayers,
        PlayersState,
    ...
    //     // debugger
    //
    //     undoList = convertPlainObjectToState(
    //     undoList,
    //     LogLineData, defaultLogUndo, LogUndoState, OrderedMap
    //     );
    //
