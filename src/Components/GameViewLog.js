...
import {
  loadDataForShowGame, loadPlayers, loadRosters, loadViewLog,
} from '../AC';
...
class GameViewLog extends Component {
  async loadAllData() {
    const {
      loadViewLog, loadRosters, loadPlayers, loadDataForShowGame, logID, gameID,
    } = this.props;
  ...
  componentDidMount() {
    const {
      loadViewLog, loadRosters, loadPlayers, logID, isGlobalShow,
    } = this.props;
  ...
  render() {
    const {
      log, classes, players, rosterTeamOne, rosterTeamTwo, teamNames,
    } = this.props;
    ...
    const data = log.logList.map((line) => createData(
      line,
      players,
      rosterTeamOne,
      rosterTeamTwo,
...
GameViewLog.propTypes = {
  ...
  loadViewLog: PropTypes.func.isRequired,
};
...
const mapStateToProps = (state, ownProps) => {
  ...
  if (!isGlobalShow) {
    ...
    return (state, ownProps) => getLogProps(state, ownProps);
  }
};
